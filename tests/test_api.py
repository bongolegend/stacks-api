...
def test_get_user_by_email():
    with get_db() as db:
        res = api.get_user_by_email(db, email='does-not-exist@a.b')
        assert res is None
...
def test_create_follow():
    ...
    with get_db() as db:
        user1 = api.create_user(db, user1)
        user2 = api.create_user(db, user2)
        ...
        api.delete_user(db, user1.id)
        api.delete_user(db, user2.id)